...
class MemoryGameView(discord.ui.View):
    ...
    async def card_callback(self, interaction: discord.Interaction):
        ...
        if len(self.flipped) == 2:
            ...
            # ==================================================
            # Notice: Ye 'else' ab thoda peeche hai (Sahi jagah par)
...
class SequenceTaskView(HeistTaskView):
    ...
    def make_callback(self, val):
        async def cb(i):
            if i.user.id != self.player.id: return await i.response.send_message("❌ Don't touch!", ephemeral=True)
...
LEVEL_CONFIG = {
    ...
    2: {"grid": (3, 3), "diamonds": 2, "multiplier": 1.8, "name": "🟡 Easy Mode", "bombs": 7},      
    3: {"grid": (4, 3), "diamonds": 3, "multiplier": 2.5, "name": "🟠 Medium Mode", "bombs": 9},    
    4: {"grid": (4, 4), "diamonds": 3, "multiplier": 5.0, "name": "🔴 Hard Mode", "bombs": 13},      
...
class CashoutButton(discord.ui.Button):
    def __init__(self):
        super().__init__(style=discord.ButtonStyle.primary, label="CLAIM WINNINGS", emoji="💰", row=4, disabled=True)
